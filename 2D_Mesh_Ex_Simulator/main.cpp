...
int sc_main(int argc, char* argv[]) {
	...
	if(argc>=10)
	{
		...
		delta = atof(argv[11]);
		point_temp = atof(argv[12]);
		L_MR_passive = atof(argv[13]);
		routing_algo = atoi(argv[14]);
	...
	for(int i=0; i < 7; i++){
		chip_app_temp.push_back(vector<double>(processor_no, 55.0));
	}
	chip_temp = chip_app_temp[application_num];

	// for(int i=0; i < processor_no; i++){
	// 	chip_temp[i] = chip_temp_test[i];
	// }
	...
	// steady_file_name = "NOC_" + to_string(NOC_WIDTH) + "_" + to_string(NOC_HEIGHT) + ".steady";
    // ptrace_file_name = "NOC_" + to_string(NOC_WIDTH) + "_" + to_string(NOC_HEIGHT) + ".ptrace";

	// command_str = "hotspot -c hotspot.config -f noc_8_8.flp -p " \
    //                     + ptrace_file_name + " -steady_file " \
    //                     + steady_file_name + " -model_type grid";
    // init_command_str = "hotspot -c hotspot.config -init_file " + steady_file_name + " -f noc_8_8.flp -p " \
    //                     + ptrace_file_name + " -steady_file " \
    //                     + steady_file_name + " -model_type grid";  

	// com_char = command_str.c_str();
	// init_com_char = init_command_str.c_str();
	cout << "delta: " << delta << endl;
	cout << "routing_algo: " << routing_algo << endl;
	//remove(ptrace_file_name);
	//remove(steady_file_name);
	...
	// proc_state_name = fold_name + type_name + "_proc_state.txt";
	// const char* c_proc_state_name = proc_state_name.c_str();
	// proc_state_log.open(c_proc_state_name, ios::out| ios::app);
	// cout << "proc_state_name: " << proc_state_name << endl;
	ptrace_file.open(ptrace_file_name, ios::out| ios::trunc);


    for(int i = 0; i < NOC_WIDTH; i++){
        for(int j = 0; j < NOC_HEIGHT; j++){
            ptrace_file << "P_" << i << "_" << j<< "\t"; 
        }
    }
	ptrace_file << endl;

...
void electronic_chip::write_state(){
	while(true){
		wait(0.4, SC_NS);
		for(int i=0; i < processor_no; i++) 
		{
			if(((*proc_list[i]).get_state() == 1)){	
				ptrace_file << "0.1092" << "\t"; 
			}else if (((*proc_list[i]).get_state() == 2)){	
				ptrace_file << "1.9" << "\t"; 
			}
			else{
				ptrace_file << "0" << "\t";
...
void electronic_chip ::update_temp(){
	while(true){
		wait(0.4, SC_NS);
		if(noc_update_timer == 1){
			noc_update_timer = UPDATE_TIMER;
			cout << "update timer time is " << sc_time_stamp().to_double() / 1000 << endl;
			// close the ptrace file
			ptrace_file.close();

			// try to open steady file
			steady_file.open(steady_file_name, ios::in | ios::out);
			if(!steady_file){
				cout << "start to command 1: " << endl;
				cout << com_char << endl;
				system(com_char);
			}else{
				steady_file.close();
				cout << init_com_char << endl;
				cout << "start to command 2: " << endl;
				system(init_com_char);
			}
			//update the chip_temp
			steady_file.open(steady_file_name, ios::in | ios::app);   
			for(int i=0; i < processor_no; i++){
				string input;
				getline(steady_file, input);
				double temp = strTod(input);
				chip_temp[i] = temp - 298.15 + 55;
				temp_trace_log << chip_temp[i] << "\t";
			}
			temp_trace_log << endl;

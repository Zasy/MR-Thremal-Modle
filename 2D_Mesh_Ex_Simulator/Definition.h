...
// state for hotspot files
fstream steady_file;
fstream ptrace_file;


string steady_file_name = "NOC_8_8.steady";
string ptrace_file_name = "NOC_8_8.ptrace";

string command_str = "hotspot -c hotspot.config -f noc_8_8.flp -p " \
					+ ptrace_file_name + " -steady_file " \
					+ steady_file_name + " -model_type grid";
string init_command_str = "hotspot -c hotspot.config -init_file " + steady_file_name + " -f noc_8_8.flp -p " \
					+ ptrace_file_name + " -steady_file " \
					+ steady_file_name + " -model_type grid";
const char* com_char = command_str.data();
const char* init_com_char = init_command_str.data();

const int UPDATE_TIMER = 10000;

double strTod(string input_s){
    return stod (input_s.substr(input_s.size()-6));
}
...
double get_router_loss(int in, int out, int id){
	...
	double T_mid = chip_temp[id];
	double active_loss = L_SW_active(T_SRC, T_mid);
	return (N_active*active_loss + N_passive*L_MR_passive + N_crossing*L_crossing);
}
